...
// ─── Arg types (mirror each tool's inputSchema) ───────────────────────────────
...
interface AssignCaseArgs {
  caseId: number;
  assignee: string;
}

interface ListUserCasesArgs {
  assignee?: string;
  status?: string;
  limit?: number;
}
...
interface ReopenCaseArgs {
  caseId: number;
  comment?: string;
}

interface CloseCaseArgs {
  caseId: number;
  comment?: string;
}

interface ListMilestonesArgs {
  ixProject?: number;
}

interface ListStatusesArgs {
  ixCategory?: number;
}

interface ViewProjectArgs {
  ixProject: number;
}

interface ViewAreaArgs {
  ixArea: number;
}
...
export async function createCase(api: IFogBugzClient, args: CreateCaseArgs | undefined): Promise<string> {
  const { title, description, project, area, milestone, priority, assignee } = args ?? {};

  if (!title) return JSON.stringify({ error: 'title is required' });

  const params: CreateCaseParams = { sTitle: title };
...
export async function updateCase(api: IFogBugzClient, args: UpdateCaseArgs | undefined): Promise<string> {
  const { caseId, title, description, project, area, milestone, priority } = args ?? {};

  if (!caseId) return JSON.stringify({ error: 'caseId is required' });

  const params: EditCaseParams = { ixBug: caseId };
...
export async function assignCase(api: IFogBugzClient, args: AssignCaseArgs | undefined): Promise<string> {
...
export async function listUserCases(api: IFogBugzClient, args: ListUserCasesArgs | undefined): Promise<string> {
  ...
  try {
    ...
    const cases = await api.searchCases({
      q: query,
      cols: ['ixBug', 'sTitle', 'sStatus', 'sPriority', 'sProject', 'sArea', 'sFixFor'],
      max: limit ?? 20,
    });
...
export async function searchCases(api: IFogBugzClient, args: SearchCasesArgs | undefined): Promise<string> {
...
export async function getCaseLink(api: IFogBugzClient, args: GetCaseLinkArgs | undefined): Promise<string> {
  const { caseId } = args ?? {};
...
export async function getCase(api: IFogBugzClient, args: GetCaseArgs | undefined): Promise<string> {
...
export async function resolveCase(api: IFogBugzClient, args: ResolveCaseArgs | undefined): Promise<string> {
...
export async function reopenCase(api: IFogBugzClient, args: ReopenCaseArgs | undefined): Promise<string> {
...
export async function closeCase(api: IFogBugzClient, args: CloseCaseArgs | undefined): Promise<string> {
  const { caseId, comment } = args ?? {};
...
export async function listMilestones(api: IFogBugzClient, args: ListMilestonesArgs | undefined): Promise<string> {
  try {
...
export async function listStatuses(api: IFogBugzClient, args: ListStatusesArgs | undefined): Promise<string> {
  try {
...
export async function viewProject(api: IFogBugzClient, args: ViewProjectArgs | undefined): Promise<string> {
...
export async function viewArea(api: IFogBugzClient, args: ViewAreaArgs | undefined): Promise<string> {
  const { ixArea } = args ?? {};
...
export async function apiRequest(api: IFogBugzClient, args: ApiRequestArgs | undefined): Promise<string> {
...
export async function createProject(api: IFogBugzClient, args: CreateProjectArgs | undefined): Promise<string> {
  const { name, primaryContact, isInbox, allowPublicSubmit } = args ?? {};

  if (!name) return JSON.stringify({ error: 'name is required' });

  try {
    const params: CreateProjectParams = { sProject: name };
